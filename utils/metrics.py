...
class AccTopk():
    ...
    def update(self, predict, target):
        ...
        # 分离背景类和非背景类
        background_mask = (target == self.background_classes)
        non_background_mask = ~background_mask
        
        # 背景类必须精确匹配
        bg_correct = np.sum(predict[background_mask] == target[background_mask])
        
        # 非背景类使用容错匹配
        if np.any(non_background_mask):
            nb_correct = np.sum(np.absolute(predict[non_background_mask] - target[non_background_mask]) < self.k)
        else:
            nb_correct = 0
            
        self.topk_correct = bg_correct + nb_correct
    
    def get(self):
        return self.topk_correct * 1.0 / self.cnt if self.cnt > 0 else 0.0
