...
def get_args():
    ...
    parser.add_argument('--data_root', default = r"D:\Users\zr159\Desktop\Ultra-Fast-Lane-Detection-master\datasets\pic", type = str)
    ...
    parser.add_argument('--test_model', default = "./culane_18.pth", type = str)
...
def save_training_checkpoint(path, *, model, optimizer, scaler, epoch,
                             val_loss, best_val_loss,
                             global_optimizer_step, options):
    """Atomically save all state required to resume a fine-tuning run."""
    output = Path(path)
