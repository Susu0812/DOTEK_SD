...
from data.constant import tusimple_row_anchor, culane_row_anchor, my_row_anchor
# 导入库
...
class TestDataset_png(torch.utils.data.Dataset):  # 加载测试数据集----------------------------------------------------------
    def __init__(self, path, img_transform=None):
        super(TestDataset_png, self).__init__()
        self.path = path
...
def parse_opt():  # 参数指定-------------------------------------------------------------------------------------------
    ...
    parser.add_argument('--model', type=str, default='./logs/1023_1505_lr_1e-04_b_64/ep099.pth', help='模型路径')  # 设置
    parser.add_argument('--dataset', type=str, default='datasets', help='数据集名称')
    parser.add_argument('--source', type=str, default='./datasets/test/pic', help='测试路径')  # 设置
    parser.add_argument('--savepath', type=str, default='./datasets/show', help='保存路径')  # 设置
    parser.add_argument('--save_video', type=bool, default=False, help='保存为视频')
    parser.add_argument('--griding_num', type=int, default=80, help='网格数')   #200,18,4     100,56,4
    parser.add_argument('--num_row_anchors', type=int, default=24, help='锚框行')
...
def run(opt):
    ...
    for dataset in zip(datasets):  # splits：图片列表 datasets：统一格式之后的数据集
        loader = torch.utils.data.DataLoader(dataset, batch_size=1, shuffle=False, num_workers=0)  # 加载数据集
        if save_video:
            fourcc = cv2.VideoWriter_fourcc(*'MJPG')
            vout = cv2.VideoWriter(opt.dataset + '.avi', fourcc, 30.0, (img_w, img_h))  # 保存结果为视频文件
        else:vout=None
